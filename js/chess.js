...
function material(color){if(!materials.has(color)){const value=new THREE.MeshStandardMaterial({color,roughness:color===BLACK?.72:.64,metalness:color===GOLD?.42:.02});value.userData.preserveColor=true;materials.set(color,value);}return materials.get(color);}
...
function crown(root,type,mat){
 ...
 if(type==='bishop'){const neck=lathe([[.08,.32],[.12,.38],[.09,.44],[.125,.49],[.075,.54],[0,.58]],mat),mitre=new THREE.Mesh(new THREE.SphereGeometry(.085,20,14),mat);mitre.position.y=.51;mitre.scale.y=1.22;root.add(neck,mitre);return;}
