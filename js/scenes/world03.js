...
import {addObject,applyResponsivePriority,BILLBOARD,disposeRoot,plane,texture} from './composition.js';
...
export function createWorld03(){
  ...
  const specs=[
    {group:midground,x:.82,y:.08,z:-3.05,scale:1.82,tilt:-.08,phase:.4,duration:8.7,opacity:.96,priority:1},
  ...
  function update(time,delta,{view={yaw:0,pitch:0},camera}={}){animated.forEach(object=>{const u=object.userData,w=Math.PI*2/u.duration,wave=Math.sin(time*w+u.phase),yaw=THREE.MathUtils.clamp(view.yaw||0,-.42,.42),pitch=THREE.MathUtils.clamp(view.pitch||0,-.32,.32);object.position.set(u.base.x-yaw*u.parallax*1.5,u.base.y+wave*u.drift-pitch*u.parallax*1.5,u.base.z);object.rotation.z=u.baseRotation.z+wave*(.018+(u.priority===1?.009:0));if(camera&&u.mode===BILLBOARD.Y_AXIS)object.rotation.y=camera.rotation.y;});clocks.forEach((clock,i)=>clock.traverse(object=>{if(object.userData.clockHand)object.rotation.z=object.userData.baseAngle+time*(object.userData.clockHand==='minute'?.045:.006)+i*.04;}));}
  ...
  return{root,update,responsive,reset(){animated.forEach(o=>o.position.copy(o.userData.base));},dispose(){disposeRoot(root);}};
}
