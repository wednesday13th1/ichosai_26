...
export function createWorld01(){
 ...
 let theme=null;function update(time,delta,{theme:nextTheme,camera}={}){const active=nextTheme||theme||{animationSpeed:1},t=visit.elapsed(time)*active.animationSpeed;animated.forEach((object,i)=>{const u=object.userData,m=microMotion(t,{phase:u.phase,duration:4.4+i%4*.65,float:(u.hero?.28:.2)*motion.scale,rotate:(u.hero?.26:.2)*motion.scale,scale:(u.hero?.35:.3)*motion.scale}),heroPulse=u.hero?Math.max(0,Math.sin(t*.62+u.phase))*.3:0;object.position.y=u.base.y+m.y;object.position.z=u.base.z+heroPulse*.32;object.position.x=u.base.x+Math.cos(t*.42+u.phase)*.07*motion.scale;if(camera){object.lookAt(camera.position);object.rotateZ(m.rotation);object.rotateX(Math.sin(t*.31+u.phase)*.08*motion.scale);object.rotateY(Math.cos(t*.27+u.phase)*.06*motion.scale);}else{object.rotation.set(u.baseRotation.x+m.rotation*.2,u.baseRotation.y,u.baseRotation.z+m.rotation);}object.scale.setScalar(u.baseScale*(m.scale+heroPulse));u.hands.forEach((hand,index)=>hand.rotation.z=hand.userData.baseAngle+t*(index?.22:.055)*(index?-1:1)*(1+(i%3)*.12));});}
