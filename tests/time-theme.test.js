...
test('time themes switch at the requested local-hour boundaries',()=>{
 ...
 assert.equal(getTimeTheme(at(20)).name,'night');
 assert.equal(getTimeTheme(at(4)).name,'night');
});
test('theme values blend during the final 30 minutes before a boundary',()=>{const before=new Date(2026,8,9,19,0),near=new Date(2026,8,9,19,50);assert.equal(getTimeTheme(before).blend,0);assert.ok(getTimeTheme(near).blend>.6);assert.equal(getTimeTheme(near).from,'evening');assert.equal(getTimeTheme(near).to,'night');});
